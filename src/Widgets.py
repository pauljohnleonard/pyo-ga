...
class CustomMenu(wx.Panel):
    def __init__(self, parent, choice=[], init='', size=(100,20), outFunction=None, colour=None, maxChar=None, columns=1,key=None):
    ...
    def setLabel(self, label, out=False):
        ...
        if self.outFunction and self.label != '' and out:
            self.outFunction(self.choice.index(self.label), self.label,self.key)
        self.Refresh()
...
class MenuFrame(wx.Frame):
    ...
    def MouseDown(self, event):
        ...
        if win != None:
            ...
            if True:   # win in [CeciliaLib.getVar("mainFrame"), CeciliaLib.getVar("interface")]:
                win.Raise()
        try:
...
class EntryUnit(wx.Panel):
    def __init__(self, parent, value=0, unit='', size=(130,20), valtype='float', outFunction=None, colour=None):
        ...
        if False: # PJL CeciliaLib.getVar("systemPlatform") == 'win32':
            self.starttext = 65
        else:    
            self.starttext = 95
        if colour:
    ...
    def OnPaint(self, event):
        ...
        if self.selected and self.new:
            ...
            val = str(self.value)
        if True:  # PJL CeciliaLib.getVar("systemPlatform") == 'linux2':
...
class ControlKnob(wx.Panel):
    def __init__(self, parent, minvalue, maxvalue, init=None, pos=(0,0), size=(50,70), log=False, outFunction=None, integer=False, backColour=None, label='',key=None):
    ...
    def OnPaint(self, evt):
        ...
        if True:   #CeciliaLib.getVar("systemPlatform") == 'linux2':
            width = len(val) * (dc.GetCharWidth() - 3)
        else:
            width = len(val) * dc.GetCharWidth()
        dc.SetTextForeground(CONTROLSLIDER_TEXT_COLOUR)
        ...
        # Send value
        if self.outFunction:
            self.outFunction(self.GetValue(),self.key)
...
class ControlSlider(wx.Panel):
    def __init__(self, parent, minvalue, maxvalue, init=None, pos=(0,0), size=(200,20), key=None,log=False, outFunction=None, integer=False, backColour=None):
        wx.Panel.__init__(self, parent=parent, id=wx.ID_ANY, pos=pos, size=size, style=wx.NO_BORDER | wx.WANTS_CHARS)
        self.SetBackgroundStyle(wx.BG_STYLE_CUSTOM)  
        self.SetBackgroundColour(BACKGROUND_COLOUR)
        self.SetMinSize(self.GetSize())
        self.knobSize = 20
        self.knobHalfSize = 10
